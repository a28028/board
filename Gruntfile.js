module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        clean: {
            contents: ['dist/*', '!**/dist/media/**']
        },
        copy: {
            files: {
                cwd: './', // set working folder / root to copy
                src: ['**/*', '.htaccess', '!**/node_modules/**', '!**/dist/**'], //''!**/node_modules/**''], //,'!**/docker-scripts/**','!**/Jelastic/**' ,'!**/dist/**','!dist/media/*'],           // copy all files and subfolders
    ...
    grunt.registerTask('set_base_root', function (env) { grunt.file.setBase('./'); });
    grunt.registerTask('set_base_dist', function (env) { grunt.file.setBase('dist'); });
    grunt.registerTask('build', 'Build task', function (env) {
        if (env == 'deploy1') {
            grunt.task.run(['set_base_root', 'clean', 'copy', 'set_base_dist',
                'jshint',
                'phplint',
                'less',
                'jst',
